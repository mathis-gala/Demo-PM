...
export function Topics({ res }: { res: ResLike }) {
  ...
  const totalSubtopics = grouped.reduce((acc, theme) => acc + theme.subtopics.length, 0);

  return (
    <div className="mt-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold tracking-tight text-slate-900">Catégories</h2>
          <p className="mt-1 text-sm text-slate-600">
            {grouped.length} catégories • {totalSubtopics} sous-catégories • {res.results.length}{' '}
            {pluralize(res.results.length, 'idée', 'idées')} • {formatTime(res.time)}
          </p>
        </div>
